...
@property (nonatomic, assign) BOOL        isUserEnteringMail;
	...
	[UIView animateWithDuration: _lockAnimationDuration animations: ^{
		if (_displayedAsLockScreen) {
			...
			if (_isUserTurningPasscodeOff) {
               ...
               // [CADataManager sharedManager].savePswd=@"";
			}
			// Update the Keychain if adding or changing passcode
			else {
				[self _savePasscode:_tempPasscode];
                
             //    [CADataManager sharedManager].savePswd=_tempPasscode;
...
-(void)confirmationMail
{
    _isUserBeingAskedForNewPasscode = NO;
    _isUserConfirmingPasscode = YES;
    _failedAttemptLabel.hidden = YES;
    _isUserEnteringMail=YES;
    _isUserChangingPasscode=NO;
...
-(void)confirmMailview
{
    
    _passcodeTextField=[[UITextField alloc]initWithFrame:CGRectMake(self.view.frame.size.width*.05, self.view.frame.size.height*.5, self.view.frame.size.width*.9, self.view.frame.size.height*.05)];
    _passcodeTextField.placeholder = @"Enter your e-mail ID";
    _passcodeTextField.autocorrectionType = UITextAutocorrectionTypeNo;
    [_passcodeTextField setFont:[UIFont fontWithName:RobotoRegular size:12.0f]];
    
    _passcodeTextField.borderStyle=UITextBorderStyleRoundedRect;
    [_passcodeTextField setDelegate:self];
...
- (void)_resetTextFields {
  ...
  if( _isUserEnteringMail)
    {
        //[_passcodeTextField resignFirstResponder];
        [self confirmMailview];
    }
}
...
- (void)_resetUI {
	...
	if (_isUserConfirmingPasscode) {
        ...
        else if(_isUserEnteringMail)
        {
            _enterPasscodeLabel.text = NSLocalizedStringFromTable(self.enterEmailIdString, _localizationTableName, @"");
        }
	}
...
- (void)_loadStringDefaults {
    ...
    self.enterEmailIdString=@"Enter your password Recovery Email-Id";
...
- (void)_loadKeychainDefaults {
    _keychainPasscodeUsername = @"PintelPasscode";
    _keychainTimerStartUsername = @"PintelPasscodeTimerStart";
    _keychainServiceName = @"PintelServiceName";
